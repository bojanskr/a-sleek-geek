 ...
 * Do not add/edit/save any files or folders iside this folder. 
 * They will be deleted by the gulp tasks.
 */
const distDir = "./dist/";
...
function compileHTML() {
  ...
  for (let vendor in vendors) {
    if (vendors[vendor].css_link !== undefined) {
      css_links +=
        '<link href="' + vendors[vendor].css_link + '" rel="stylesheet">';
    }
...
function copyVendors() {
  ...
  for (let vendor in vendors) {
    if (vendors[vendor].src) {
      stream.add(
        gulp
        .src(vendors[vendor].src)
        .pipe(gulp.dest(distDir + vendors[vendor].dest))
      );
    } else if (vendors[vendor].srcs) {
      for (let multivendor in vendors[vendor].srcs) {
        stream.add(
          gulp
          .src(vendors[vendor].srcs[multivendor].src)
          .pipe(gulp.dest(distDir + vendors[vendor].srcs[multivendor].dest))
        );
      }
    }
  }
...
const dist = gulp.series(
  clean,
  gulp.parallel(copyFiles, compileHTML, compileSCSS, compileJS, copyVendors)
);
